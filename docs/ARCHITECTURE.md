...
- SQLite 使用 WAL，保存管理员、上游账号、下游用户服务、续期历史、节点、流量明细、采集器状态、限流状态和迁移版本；
...
| `import_customer_services.py` | 从服务器本地私密 JSON 文件幂等导入用户服务，不把业务数据打包进源码 |
...
7. 上游返回流量统计时以该值更新账号总量，使月度重置能正确回落；未返回统计时保留本地采集值。
...
1. 管理员为每位用户服务生成随机分享 Token；
2. 用户可以在不更换链接的情况下迁移到另一条上游专线；
3. `/sub/<token>` 只在用户服务期和上游账号同时有效时读取上一次成功同步的数据；
4. 按 User-Agent 返回 Clash YAML 或通用 Base64；
5. 用户或账号停用、用户到期、删除记录或轮换 Token 会立即使旧 URL 失效。

### 用户服务与月度流量周期

1. 上游账号的 `expire_date` 始终保存真实到期日；
2. 应用只取日期中的日号作为月度重置日，例如 `2027-04-24` 对应每月 24 日；
3. 29、30、31 日遇到较短月份时使用当月最后一天；
4. 到达新周期后，以条件更新方式只清零一次本地上传、下载和累计流量，不修改真实到期日；
5. 用户服务单独保存开始日和到期日，续期操作写入历史，仪表盘「需要处理」合并用户续费、账号到期、流量和节点异常；到期类汇总未来 30 天及已过期项目，分组折叠并在列表内滚动。
6. 所有业务日期按 `Asia/Shanghai` 自然日判断；数据库审计和更新时间戳继续使用 UTC，避免混淆日期规则与日志时间线。
...
- 服务端渲染减少了前端构建链和供应链依赖；
- 批量任务同时使用线程锁和与数据库路径关联的 OS 文件锁，支持同机多个 Gunicorn Worker 互斥；进程退出会释放文件锁，不是多机分布式任务队列；
- 续费提醒目前显示在管理后台并记录“已提醒”时间，不主动发送短信、邮件或聊天消息；外部通知需要由部署方提供明确的通知渠道；
...
`subscription_export.py` 统一执行屏蔽、依赖解析、保留名称避让、重命名及输出预算；页面预览与两类订阅使用相同结果。`proxy_verifier.py` 管理受限核心进程、固定地址与私有控制套接字。`node_monitor.py` 提供每分钟运行的有界任务，复用跨进程锁与节点租约，晚到结果不会写入被替换的配置。

正式发布由 `install-release.sh` 先验证签名再执行归档内的 `deploy.sh`。部署、定时备份、节点复测分别有独立的生命周期与资源预算；节点复测通过主服务绑定在更新时停止，旧版本没有脚本时自动跳过。
