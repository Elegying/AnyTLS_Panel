...
| `ANYTLS_REPO_REF` | `v1.4.12` | 显式设置时优先从仓库拉取；`vX.Y.Z` 必须为真实标签且与源码版本一致；生产环境使用正式标签 |
...
管理员进入「订阅设置」，开启「节点屏蔽」并填写关键词，每行一个。
匹配上游原始节点名称（重命名前），忽略英文字母大小写；包含任意关键词即不输出到订阅。
最多 100 个关键词，每个最多 128 字，总输入最多 8192 字；空行忽略，留空不屏蔽。
规则适用于所有账号、用户分享订阅及管理端合并订阅，兼容通用 Base64 和 Clash 输出。
保存后用户下次更新订阅时生效，不删除面板节点；关闭开关可恢复输出。
如果全部节点匹配，返回成功的空订阅，不回退输出被屏蔽节点；部分客户端可能保留旧节点，需检查客户端更新结果。
